...
const paths = {
    ...
    layout: path.join(__dirname, 'pages', 'layout.html'),
...
async function renderPage(layoutHtml, pageName) {
    ...
    await fs.writeFile(path.join(paths.docs, pageName), html, 'utf8');
}
...
async function build() {
    try {
        await ensureCleanDocs(paths.docs);
